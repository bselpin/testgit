...
const EventPage = ({ router }) => {
	...
	const meta = {
		title: "이벤트 | 활력선물 이벤트",
		description:
			"요양 관련 궁금했던 질문을 남겨주시면, 건강한 활력을 담은 선물과 함께 또하나의가족이 도와드릴게요",
		url: `${process.env.NEXT_PUBLIC_BASE_URL}${router.asPath}`,
		img: `${process.env.NEXT_PUBLIC_BASE_URL}/event/power_01.jpg`,
	}
	...
	return (
		<>
			...
			<DetailHeader name={"활력선물 이벤트"} meta={meta} router={router} share={share} />

			<div className={style.event_wrap}>
				<div className={style.event_img_box}>
					<img src="/event/power_01.jpg" alt="활력선물 이벤트" />
					<a
						href="http://naver.me/GSi5wHfe"
						target="_blank"
						rel="noopener noreferrer"
						className={`${style.link} ${style.power}`}
					>
						<span>소문내기</span> 이벤트 참여하기
						<span className={style.arrow}>{">"}</span>
					</a>
				</div>
				<div className={style.event_img_box}>
					<img src="/event/power_02.jpg" alt="활력선물 이벤트" />
				</div>
			</div>
		</>
	)
